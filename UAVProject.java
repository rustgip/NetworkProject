...
public class UAVProject {
	...
	static ArrayList<Double> avgTime = new ArrayList<Double>();
	...
	public static void main(String[] args) throws InterruptedException, IOException {
		...
		int networkTime = 1000; // Total number of executions
		double executionTime = 5; // Loop time in ms - 1000 = 1 second, change
		...
		// Spinning up threads for each simulation...
			...
			Network n = generateNForEmandEd(networkTime);
		...
		x.schedule(new TimerTask() {
			...
			public void run() {
				...
				for (int run = 0; run < avgTime.size(); run++) {

					totalavg = totalavg + avgTime.get(run);

				}

				System.out.println("---------------\n" + totalavg/avgTime.size());
				System.exit(0);
	...
	// generateNFormEmandEd - This function just generates a network (i.e.
	// Nodes, Base Stations, etc...) with the input network parameters...
	// The returned network is passed to the CreateSimEnvironment and utilized
	// when actually starting the simulation.
	public static Network generateNForEmandEd(int networkTime) {

		Network n = new Network(500, 500, networkTime);
	...
	// selectBaseDestination - Called by CreateSimEnvironment. Gives each
	// dynamic node a base station (destination) to move to.
	//
	// Issue: Why are we passing the network to the function if it is not used?
	// Commented for now...
	// Also, we should just prevent the base station issue earlier! Rather than
	// blocking them here possibly...
	...
	// takes all the nodes and adjusts their routes in order to maximize wifi
	// connectivity
	public static void routeAdjustments(Network n, ArrayList<Node> movingNodes, double dynamicNodeThreshold,
			double destinationThreshold) {
		...
		for (Node node : movingNodes) {
			...
			if (!hasBaseStationNeighbor(node)) {
				...
				// if node current distance from destination is within a current
				// threshold then just head straight to destination
				...
				// otherwise path adjustment towards dynamic nodes with wifi
				// connection
				else {

					// finds node with closest destination to current node to
					// use that nodes path to follow
					nodeToFollow = findNodeWithClosestDestination(n, node);

					if (nodeToFollow != null) {

						// dynamicNodeThreshold - needs to looked into further
						// checks to see if the destinations of nodeToFollow and
						// current node aren't too far apart
						// or else path adjustments will take the node in the
						// wrong direction
						double destinationDistanceApart = euclideanDistance(node.endX, nodeToFollow.endX, node.endY,
								nodeToFollow.endY);

						// if threshold met then continue adjusting angle to
						// follow other node
						...
						// if threshold not met then set angle to head directly
						// towards destination
						else {
							Coordinates currentLocation = new Coordinates(node.currentX, node.currentY);
							Coordinates destinationLocation = new Coordinates(node.endX, node.endY);
	...
	// returns the adjusted angle taken from midway-point between destination of
	// the new nodes
	...
	// checks neighbors of the node to see which of its neighbors destination is
	// closest to current nodes destination and is also connected to wifi
	public static Node findNodeWithClosestDestination(Network n, Node currNode) {
		...
		// loops through set of neighbor nodes to determine which one has the
		// closest destination to the current node's destination
	...
	public static boolean hasBaseStationNeighbor(Node currNode) {
		...
		// loops through set of neighbor nodes to determine if one is a base
		// station
		for (Node i : neighbors) {
	...
	public static HashSet<Node> inRangeWifi(Network n, HashSet<Node> finalList, HashSet<Node> currNs, long simStart) {
		...
		for (int outer = 0; outer < currNeighbs.size(); outer++) {
			...
			// @Zach: This could be doubling down on the nodes in here... Double
			// check...
			finalList.add(x);
...
//							System.out.println("Node #" + x.nodeNum + " is also in connectedToWifi range of: "
//									+ y.nodeNum + " with distance: " + distance);
	...
	public static boolean inRangeBaseStation(Network n, Node curr, ArrayList<Node> baseNodeList, long simStart) {
		...
		// Checks if in range of ANY base station for connectedToWifi purposes,
		// not
		// destination
...
//					System.out.println("In range of non target base.\n");
	...
	public static void CreateSimEnviron(Network n) throws IOException {
		...
		// Determining and setting the angle at which the nodes must travel to
		// reach the destination base station...
		//
		// Rerouting: Setting angle is going to be important for our rerouting
		// process so this is a good example of implementation...
		...
		for (int baseStationIndex = 0; baseStationIndex < n.baseNodes.size(); baseStationIndex++) {
			...
			baseStationOut.write(
					tempBaseStation.nodeNum + "," + tempBaseStation.currentX + "," + tempBaseStation.currentY + "\n");
		}
		...
		for (int loops = 0; loops < n.getRunTime(); loops++) {

				// Determines total sim executions
				//
				// Rerouting: totalsimrunning is set to 0 initially and is only
				// incremented here...
				// I am really confused by the use of System.currentTImeMillis
				// here though.
				// Im not sure how youre making sure this works correctly.
				// To be honest, Im not sure why System time us used at all...
				// Its hard to follow.
...
//					System.out.println("Elapsed time: " + (System.currentTimeMillis() - time) + "----------\n");
					...
					// Setting up to print out the output CSV...
					try {
						out = new PrintWriter(new FileWriter("Test" + totalRun + ".csv", true));
						movementOut = new PrintWriter(new FileWriter("movement_plot" + totalRun + ".csv", true));
					} catch (IOException e) {
					...
					// Its seeming like a lot of string building for logging
					// purposes...
					for (int movePos = 0; movePos < n.movingNodes.size(); movePos++) {

						// We clear the list of neighborNodes for some reason...
						//
						// Pulkit says to clear the list of neighbor nodes after
						// every iteration...
						// Apparently every other list in the simulation gets
						// purged after each iteration...
						...
						// Traveling node's positions are incremented. -
						// Traveling is default.
						...
						// If its not labeled as traveling, then it must be
...
//								System.out.println("Pausing");
							...
							// If pause time over, reset movement parameters and
							// set too traveling status.
							else {
...
//								System.out.println("Unpausing\n");
						...
						// If the current node is within range of its
						// destination base station, add it to the list of
						// arrived nodes...
						//
						// Rerouting: We may need to add additional code here!
						// We would want to not only check inRageBaseStation,
						// but also if it can hear
						// any of the other dynamic nodes and, if so, is it
						// connected to a connectedToWifi?
					...
					// For every node within the list of nodes that are within
					...
					for (Node curr : finalList) {
						...
						for (int checkMoving = 0; checkMoving < n.movingNodes.size(); checkMoving++) {
							...
							// Make sure that the node within range of
							// destination isnt the same node.
...
//											+ curr.nodeNum + " with distance: " + distance);
									...
									// Rerouting: I think we want to set the
									// current node's connectedToWifi-connected
									// flag to true.
									// I think we also want to add the node to
									// the list of connectedToWifi-connected
									// nodes in finalNodes.
					...
					// This function call takes the list of dynamic nodes that
					// are connected to a base station and then
					// updates the dynamic nodes if they are multi-hop
					// connected.
					...
					// function calls takes list of all nodes and adjusts their
					// paths
					// comment out line below to run without route adjustments
					// aka all nodes will move in straight
					// line towards destination

					routeAdjustments(n, n.movingNodes, dynamicNodeThreshold, destinationThreshold);

					// No one is connected to a base station if this if
					// statement is true!
					...
					if (fin.isEmpty()) {
						...
						// This segment of code just seems to go through the
						// list of dynamic nodes and creates a list of
						// every node that was connected at a certain time
						// interval within the nodeWifiTimes
...
//							System.out.print(node.nodeNum + " ");
							...
							n.nodeWifiTimes.put(node.nodeNum, n.nodeWifiTimes.get(node.nodeNum) + 1);
...
//						System.out.println("\n");
						...
						/*
						 * //Printing out neighbors for (int i = 0; i <
						 * existingNodes.size(); i++){ HashSet<Node> neighbors =
						 * existingNodes.get(i).getNeighborNodes();
						 * System.out.print("Node# " +
						 * existingNodes.get(i).nodeNum + " Neighbors: "); for
						 * (Node node : neighbors) {
						 * System.out.print(node.nodeNum + " "); } }
						 */
...
//						"Total simulation time: " + ((System.currentTimeMillis() - time) - 5000) / 1000 + " seconds");
				...
				avgTime.add(wifiTimeAverage);
				System.out.println(wifiTimeAverage);
...
class Node implements Serializable {
	...
	public void setAngle(double Angle) {
		...
		/*
		 * Random r = new Random(); double direction = 0 + (Math.PI - 0) *
		 * r.nextDouble(); int random = r.nextInt(10 - 1 + 1) + 1; if (random %
		 * 2 == 0) { direction = direction * -1; }
		 */
